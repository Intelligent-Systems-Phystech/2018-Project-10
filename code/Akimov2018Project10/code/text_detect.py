...
for imagePath in tqdm.tqdm(glob.glob(IMAGE_PATH + "/*.jpg"),desc=IMAGE_PATH):
    td = TextDetection(imagePath)
    res = td.detect()
    outputPath = OUTPUT_PATH + "/" + imagePath.split("/")[-1]
    plt.imsave(outputPath, td.final)
